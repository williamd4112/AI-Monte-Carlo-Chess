...
namespace mcts
{
  ...
  void sim_single_iteration(const State & state, State & next_state, int max_depth, int random_num)
  {
  ...
  int sim_until_end(const State & state, int max_depth, int max_iter)
  {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
      sim_single_iteration(last_state, next_state, max_depth, random_gen());
